...
class SyntheticNerfDataset(BaseDataset):
    def __init__(self,
        ...
        if split == 'render':
            ...
            self.poses = torch.cat([render_poses[30:31].repeat(30, 1, 1), render_poses[30:90], render_poses[90:91].repeat(30, 1, 1)])
        ...
        rays_o, rays_d, imgs = create_360_rays(
            imgs, self.poses, merge_all=split == 'train', intrinsics=intrinsics)

        super().__init__(
            datadir=datadir,
            split=split,
            scene_bbox=get_360_bbox(datadir, is_contracted=False, radius=radius),
            is_ndc=False,
            is_contracted=False,
...
def get_360_bbox(datadir, is_contracted=False, radius=1.3):
    if is_contracted:
        radius = 2
