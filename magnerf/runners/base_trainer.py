...
from magnerf.ops.image import metrics
from magnerf.ops.image.io import write_png
from magnerf.runners.regularization import Regularizer
from magnerf.ops.lr_scheduling import (
...
class BaseTrainer(abc.ABC):
    def __init__(self,
        ...
        self.valid_every = int(valid_every)
    ...
    def train_step(self, data, **kwargs) -> bool:
        ...
        with torch.cuda.amp.autocast(enabled=self.train_fp16):
            fwd_out = self.model(
                ...
                c2w=data['c2w'] if 'c2w' in data else None,
                camera_dirs=data['camera_dirs'] if 'camera_dirs' in data else None,
            ...
            if "bg_mask" in data:
                bg_acc = fwd_out['accumulation'] * data['bg_mask']
                bg_loss = torch.nn.functional.l1_loss(bg_acc, torch.zeros_like(bg_acc))
                loss = loss + 1e-4 * bg_loss
        ...
        if self.global_step % self.calc_metrics_every == 0:
            with torch.no_grad():
                ...
                self.loss_info[f"lr"].update(self.lr)
                for r in self.regularizers:
    ...
    def _move_data_to_device(self, data):
        ...
        if "bg_mask" in data:
            data["bg_mask"] = data["bg_mask"].to(self.device)
    ...
    def evaluate_metrics(self,
        ...
        for k in preds.keys():
            if "depth" in k:
                ...
                out_depth = torch.cat((out_depth, prop_depth), dim=1) if out_depth is not None else prop_depth
...
def initialize_model(
    ...
    if runner.train_dataset is not None:
        ...
        try:
            ...
            num_images = None
    if hasattr(dset, "use_app_emb"):
        use_app_emb = getattr(dset, "use_app_emb")
    else:
        use_app_emb = False
    if use_app_emb:
    ...
    model = LowrankModel(
        ...
        use_appearance_embedding=use_app_emb,
