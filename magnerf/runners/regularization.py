...
from magnerf.ops.losses.histogram_loss import interlevel_loss
from magnerf.raymarching.ray_samplers import RaySamples
...
class PlaneTV(Regularizer):
    def __init__(self, initial_value, what: str = 'field'):
        if what not in {'field', 'motion_field', 'proposal_network'}:
            raise ValueError(f'what must be one of "field", "motion_field", or "proposal_network" '
    ...
    def _regularize(self, model: LowrankModel, **kwargs):
        ...
        elif self.what == 'motion_field':
            multi_res_grids = model.field.motion_grids
        else:
        ...
        for grids in multi_res_grids:
            ...
            for grid_id in spatial_grids:
                _grid = grids[grid_id]
                if len(_grid.shape) > 4:
                    for t in range(_grid.shape[-1]):
                        total += compute_plane_tv(_grid[..., t])
                else:
                    total += compute_plane_tv(_grid)
            for _grid in grids:
                # grid: [1, c, h, w]
                if len(_grid.shape) > 4:
                    for t in range(_grid.shape[-1]):
    ...
    def _regularize(self, model: LowrankModel, **kwargs) -> torch.Tensor:
        ...
        for grids in multi_res_grids:
            ...
            for grid_id in time_grids:
                ...
                if len(_grid.shape) > 4:
                    gs = _grid.shape
                    B, C, T = gs[0], gs[1], gs[-1]
                    total += compute_plane_smoothness(_grid.view(B, C, -1, T).permute(0, 1, 3, 2))
                else:
                    total += compute_plane_smoothness(_grid)
...
class L1MotionField(Regularizer):
    def __init__(self, initial_value):
        super().__init__('l1-motion-field', initial_value)

    def _regularize(self, model: LowrankModel, **kwargs) -> torch.Tensor:
        grids = model.field.motion_grids
        total = 0.0
        for pn_grids in grids:
            for grid in pn_grids:
                total += torch.abs(grid).mean()
...
class L1GridChange(Regularizer):
    def __init__(self, initial_value):
        super().__init__('l1-dgrid', initial_value)

    def _regularize(self, model: LowrankModel, **kwargs) -> torch.Tensor:
        grids = model.field.delta_grids
        total = 0.0
        for pn_grids in grids:
            for grid in pn_grids:
                total += torch.abs(grid).mean()
...
class TVGridChange(Regularizer):
    def __init__(self, initial_value):
        super().__init__('tv-dgrid', initial_value)
